...
class UserController extends PublicController {
	...
	public function my_apply(){
		...
		$apply = M('apply')->where('uid='.$uid)->select();
		foreach($apply as $k => $v){
			$apply[$k]['apply_time'] = date("Y-m-d",$v['apply_time']);
		}
		echo json_encode(array('status'=>1,'list'=>$apply));
		exit();
	}
	...
	public function apply_detail(){
		$id = intval($_REQUEST['apply_id']);
		$apply = M('apply')->where('id='.$id)->find();
		echo json_encode(array('status'=>1,'apply'=>$apply));
		exit();
	}
	//***************************
	//  用户 我的求购
