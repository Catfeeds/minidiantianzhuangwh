...
class IndexController extends PublicController {
    ...
    public function index(){
        ...
        //首页分类6个
        //======================
        $cat = M('sccat')->where('type=1')->order('sort desc')->limit(6)->select();
        foreach ($cat as $k => $v) {
            $cat[$k]['img'] = __DATAURL__.$v['photo_x'];
        }

    	//======================
    	//首页推荐产品
    	//======================
    	$pro_list = M('product')->where('del=0 AND type=1')->order('sort desc,id desc')->field('id,name,photo_x,price_yh')->limit(8)->select();
        ...
        //首页推荐企业
        //======================
        $shangchang = M('shangchang')->where('status=1 AND type=1')->order('sort desc,id desc')->select();
        foreach ($shangchang as $k => $v) {
            $shangchang[$k]['logo'] = __DATAURL__.$v['logo'];
        }
    	...
    	echo json_encode(array('ggtop'=>$ggtop,'prolist'=>$pro_list,'cat'=>$cat,'news'=>$news,'gong'=>$gong,'qiu'=>$qiu,'shangchang'=>$shangchang));
    	exit();
    }
